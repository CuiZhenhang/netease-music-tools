...
命令 `merge` 可用于更方便地新增文件，而不重新扫描所有文件。具体来讲，把要新增的文件单独放在一个文件夹中，在匹配好并且各种操作之后，可以用 `merge` 合并到全部音乐所在的文件夹。
...
# 根据匹配信息，将两个音频文件夹内容合并（包括后缀名不同的文件，如歌词）
node src/app.js merge <音频文件夹路径> <要被合并的音频文件夹路径>
```
...
# 根据匹配信息，将 ./unresolved 文件夹的内容合并到当前目录
node src/app.js merge . ./unresolved
...
- `merge`
  ...
  - `<source>`: 来源文件夹
  - `--copy, -c`: 复制文件而不是移动（可选）
  - `--overwrite, -o`: 覆盖已存在的文件（可选）
  - `--musicFileOnly, -m`: 仅处理音乐文件（可选）
